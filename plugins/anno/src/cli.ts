...
const ALL_HOSTS: InstallHost[] = ['codex', 'claude', 'workbuddy', 'codebuddy', 'cursor', 'antigravity', 'windsurf', 'copilot', 'muse'];
...
function parseHosts(value: string): InstallHost[] {
  ...
  if (requested.includes('all')) return ALL_HOSTS.filter(host => host !== 'muse');
  for (const host of requested) {
    if (!ALL_HOSTS.includes(host as InstallHost)) throw new Error(`Unknown host: ${host}`);
  }
  return [...new Set(requested as InstallHost[])];
}

async function chooseHosts(args: string[]): Promise<InstallHost[]> {
  const explicit = valueAfter(args, '--host') ?? valueAfter(args, '--hosts');
  if (explicit) return parseHosts(explicit);
  const detected = detectedHosts();
  if (!input.isTTY || !output.isTTY) {
    if (detected.length === 0) throw new Error('No hosts detected. Pass --host cursor,windsurf (or another comma-separated list).');
    return detected;
  }
...
async function optionsFrom(args: string[]): Promise<InstallOptions> {
  const scopeValue = valueAfter(args, '--scope') ?? 'user';
  if (scopeValue !== 'user' && scopeValue !== 'project') throw new Error('--scope must be user or project.');
  const scope = scopeValue as InstallScope;
...
function printChanges(changes: InstallChange[]): void {
