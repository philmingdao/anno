...
function resolvedHome(options: InstallOptions): string {
  return path.resolve(options.home ?? homedir());
}

function resolvedProject(options: InstallOptions): string {
  return path.resolve(options.projectDir ?? DEFAULT_PROJECT);
}

function npxServer(host: InstallHost): Record<string, unknown> {
  const common = {
    command: 'npx',
...
function jsonTarget(host: InstallHost, options: InstallOptions): JsonTarget | undefined {
  ...
  switch (host) {
    ...
    case 'muse':
      if (!options.customConfig) return undefined;
      return {
...
function skillTarget(host: InstallHost, options: InstallOptions): string | undefined {
  const home = resolvedHome(options);
  const project = resolvedProject(options);
  const user = options.scope === 'user';
  switch (host) {
...
function antigravityRoots(options: InstallOptions): string[] {
...
async function backupFile(target: string, dryRun: boolean): Promise<string | undefined> {
  if (!(await exists(target))) return undefined;
  const backup = `${target}.anno-backup-${timestamp()}`;
  if (!dryRun) await copyFile(target, backup);
  return backup;
}

async function atomicWrite(target: string, content: string, dryRun: boolean): Promise<void> {
  if (dryRun) return;
...
async function updateJson(target: JsonTarget, removeEntry: boolean, dryRun: boolean): Promise<InstallChange> {
  ...
  if (updated === original) return { host: 'cursor', action: 'unchanged', target: target.file };
...
async function installSkill(host: InstallHost, target: string, dryRun: boolean): Promise<InstallChange> {
  const source = await readFile(SKILL_SOURCE, 'utf8');
  let original: string | undefined;
  try { original = await readFile(target, 'utf8'); } catch { original = undefined; }
  if (original === source) return { host, action: 'unchanged', target };
...
async function removeSkill(host: InstallHost, target: string, dryRun: boolean): Promise<InstallChange> {
  if (!(await exists(target))) return { host, action: 'unchanged', target };
  const backup = await backupFile(target, dryRun);
  if (!dryRun) await rm(path.dirname(target), { recursive: true, force: true });
  return { host, action: 'removed', target: path.dirname(target), ...(backup ? { backup } : {}) };
}

async function installAntigravity(options: InstallOptions, removeEntry: boolean): Promise<InstallChange[]> {
  ...
  for (const root of antigravityRoots(options)) {
    ...
    if (removeEntry) {
      if (await exists(root)) {
        ...
        if (!options.dryRun) await rm(root, { recursive: true, force: true });
    ...
    for (const [target, contents] of [[manifest, manifestContents], [config, configContents], [skillPath, skill]] as const) {
...
function readCommandJson(command: string, args: string[]): unknown {
  const result = spawnSync(command, args, { encoding: 'utf8', shell: process.platform === 'win32' });
  if (result.status !== 0) return undefined;
  try { return JSON.parse(result.stdout); } catch { return undefined; }
}

async function withTimeout<T>(operation: Promise<T>, label: string, milliseconds = 15_000): Promise<T> {
...
function installNativeHost(host: 'codex' | 'claude' | 'workbuddy' | 'codebuddy', options: InstallOptions, removeEntry: boolean): InstallChange[] {
  if (!commandExists(host)) throw new Error(`${host} executable was not found. Install ${host} first, then rerun Anno setup.`);
  ...
  if (host === 'codex') {
    ...
    const marketplaceState = readCommandJson('codex', ['plugin', 'marketplace', 'list', '--json']) as { marketplaces?: Array<{ name?: string }> } | undefined;
    ...
    const pluginState = readCommandJson('codex', ['plugin', 'list', '--json']) as { installed?: Array<{ pluginId?: string; name?: string }> } | undefined;
  ...
  if (host === 'claude') {
    ...
    const marketplaces = readCommandJson('claude', ['plugin', 'marketplace', 'list', '--json']) as Array<{ name?: string }> | undefined;
...
async function applyHost(host: InstallHost, options: InstallOptions, removeEntry: boolean): Promise<InstallChange[]> {
  if (host === 'codex' || host === 'claude' || host === 'workbuddy' || host === 'codebuddy') {
    return installNativeHost(host, options, removeEntry);
  }
  if (host === 'antigravity') return installAntigravity(options, removeEntry);
  ...
  if (skill) changes.push(removeEntry
    ? await removeSkill(host, skill, Boolean(options.dryRun))
    : await installSkill(host, skill, Boolean(options.dryRun)));
  return changes;
}

export async function setup(options: InstallOptions): Promise<InstallChange[]> {
  const changes: InstallChange[] = [];
  for (const host of options.hosts) changes.push(...await applyHost(host, options, false));
  if (!options.skipDoctor && !options.dryRun) changes.push(...await doctor(options));
  return changes;
}

export async function uninstall(options: InstallOptions): Promise<InstallChange[]> {
  const changes: InstallChange[] = [];
  for (const host of options.hosts) changes.push(...await applyHost(host, options, true));
  return changes;
}

export async function doctor(options: InstallOptions): Promise<InstallChange[]> {
  const changes: InstallChange[] = [];
  for (const host of options.hosts) {
    if (host === 'antigravity') {
      for (const root of antigravityRoots(options)) {
        for (const required of ['plugin.json', 'mcp_config.json', path.join('skills', 'review-html-artifacts', 'SKILL.md')]) {
          ...
          if (!(await exists(target))) throw new Error(`Anno ${host} installation is incomplete: ${target} is missing.`);
        }
    ...
    const target = jsonTarget(host, options);
    if (target) {
      if (!(await exists(target.file))) throw new Error(`Anno ${host} configuration is missing: ${target.file}`);
      const parsed = parse(await readFile(target.file, 'utf8')) as Record<string, unknown>;
      let cursor: unknown = parsed;
      for (const key of target.jsonPath) cursor = (cursor as Record<string, unknown> | undefined)?.[key];
      if (!cursor) throw new Error(`Anno ${host} entry is missing from ${target.file}.`);
      changes.push({ host, action: 'verified', target: target.file });
    }
  }

  const stateRoot = await mkdtemp(path.join(tmpdir(), 'anno-doctor-'));
  const env = { ...process.env, ANNO_HOST: 'generic', ANNO_DATA_DIR: stateRoot, ANNO_DISABLE_CODEX_FALLBACK: '1' } as Record<string, string>;
  ...
  try {
    ...
    for (const required of ['html_review_start_session', 'html_review_get_session', 'html_review_register_final']) {
      if (!names.has(required)) throw new Error(`Anno MCP doctor failed: ${required} is missing.`);
    }
  } finally {
...
export function detectedHosts(home = homedir()): InstallHost[] {
  const detected = new Set<InstallHost>();
  for (const host of ['codex', 'claude', 'workbuddy', 'codebuddy'] as const) {
    if (commandExists(host)) detected.add(host);
  }
  const paths: Array<[InstallHost, string]> = [
  ...
  for (const [host, target] of paths) {
    if (spawnSync(process.execPath, ['-e', 'require("fs").accessSync(process.argv[1])', target], { stdio: 'ignore' }).status === 0) detected.add(host);
  }
  return [...detected];
}
