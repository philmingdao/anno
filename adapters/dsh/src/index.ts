...
function profileName(options: DshOptions): string {
...
function resolvedDshHome(options: DshOptions): string {
...
function dshCommand(): CommandSpec {
  if (commandExists('dsh')) return { command: 'dsh', prefix: [] };
  if (!commandExists('npx')) throw new Error('Neither dsh nor npx was found. Install Node.js 22 or newer first.');
  return { command: 'npx', prefix: ['-y', `--package=@deepseek-ai/dsh@${DSH_VERSION}`, 'dsh'] };
}

function runDsh(args: string[], options: DshOptions, capture = false): string {
...
async function sameTree(left: string, right: string): Promise<boolean> {
  ...
  for (const entry of leftEntries) {
    if (!rightNames.has(`${entry.isDirectory() ? 'd' : 'f'}:${entry.name}`)) return false;
    const leftPath = path.join(left, entry.name);
    const rightPath = path.join(right, entry.name);
    if (entry.isDirectory()) {
      if (!(await sameTree(leftPath, rightPath))) return false;
    } else if (entry.isFile()) {
      if ((await readFile(leftPath)).compare(await readFile(rightPath)) !== 0) return false;
    }
  }
  return true;
}

async function installSkill(options: DshOptions): Promise<DshChange> {
  const target = path.join(resolvedDshHome(options), 'skills', 'review-html-artifacts');
  if (await sameTree(SKILL_ROOT, target)) return { action: 'unchanged', target };
...
async function uninstallSkill(options: DshOptions): Promise<DshChange> {
  const target = path.join(resolvedDshHome(options), 'skills', 'review-html-artifacts');
  if (!(await exists(target))) return { action: 'unchanged', target };
  if (!(await sameTree(SKILL_ROOT, target)) && !options.force) return { action: 'preserved', target };
  const backup = `${target}.anno-backup-${timestamp()}`;
  if (!options.dryRun) await rename(target, backup);
  return { action: 'removed', target, backup };
}

function profileManifest(options: DshOptions): string {
  return path.join(resolvedDshHome(options), 'profiles', profileName(options), 'package.json');
}

async function verifyProfile(options: DshOptions): Promise<DshChange[]> {
  const manifestPath = profileManifest(options);
  if (!(await exists(manifestPath))) throw new Error(`DSH profile manifest is missing: ${manifestPath}`);
  ...
  for (const required of ['@philmingdao/anno-dsh', '@deepseek-ai/dsh-mcp-client', 'serverName: anno']) {
    if (!dump.includes(required)) throw new Error(`DSH composed config is missing ${required}.`);
  }
  return [
...
async function withTimeout<T>(operation: Promise<T>, label: string, milliseconds = 20_000): Promise<T> {
...
async function verifyMcp(): Promise<DshChange> {
  ...
  try {
    ...
    for (const required of ['html_review_start_session', 'html_review_get_session', 'html_review_register_final']) {
      if (!names.has(required)) throw new Error(`Anno DSH MCP doctor failed: ${required} is missing.`);
    }
  } finally {
...
export async function install(options: DshOptions = {}): Promise<DshChange[]> {
...
export async function update(options: DshOptions = {}): Promise<DshChange[]> {
  return install(options);
}

export async function uninstall(options: DshOptions = {}): Promise<DshChange[]> {
  const profile = profileName(options);
...
export async function doctor(options: DshOptions = {}): Promise<DshChange[]> {
  const skill = path.join(resolvedDshHome(options), 'skills', 'review-html-artifacts', 'SKILL.md');
  if (!(await exists(skill))) throw new Error(`Anno DSH skill is missing: ${skill}`);
  const skillStat = await stat(skill);
  if (!skillStat.isFile()) throw new Error(`Anno DSH skill is not a file: ${skill}`);
  return [
